...
//                 if($stmt1->num_rows == 1){                    
//                     // Bind result variables
//                     $stmt1->bind_result($id, $fullname, $gender, $faculty, $course, $matric_number, $graduation_year, $phone, $email, $address, $photo, $hashed_password);
//                     if($stmt1->fetch()){
//                         if(password_verify($password, $hashed_password)){
//                             // Password is correct, so start a new session
//                             session_start();
...
//                 } else{
//                     // Display an error message if email doesn't exist
//                   $_SESSION['email_err'] = "<div id='card'><h6><i class='fa fa-times-circle'></i> This email does not belong to a registered Member.</h6></div>";
//                     // $email_err = "This email does not belong to a registered Member.";
//                 }
//             } else{
//                 echo "Oops! Something went wrong. Please try again later.";
...
//         } 
        ...
        // else{
        //   $sql2 = "SELECT id, fullname, phone, email, photo, password, FROM admin WHERE email = ?";
        //     // $sql2 = "SELECT id, fullname, email, pnum, password FROM admin WHERE email = ?";
        ...
        //         if($stmt2->num_rows == 1){                    
        //             // Bind result variables
        //             $stmt2->bind_result($id, $fullname, $email, $phone, $hashed_password);
        //             if($stmt2->fetch()){
        //                 if(password_verify($password, $hashed_password)){
        //                     // Password is correct, so start a new session
        //                     session_start();
